...
class DomMesh extends Mesh {
    ...
    MouseDown(func){
        ...
        window.addEventListener( 'touchstart', (event) => {
            ...
            // calculate pointer position in normalized device coordinates
            // (-1 to +1) for both components
            const pointer = new Vector2( Math.round( ( ( event.touches[0].pageX / window.innerWidth ) * 2 - 1 )*1000 )/1000, Math.round(( - ( event.touches[0].pageY / window.innerHeight ) * 2 + 1 )*1000)/1000 );
